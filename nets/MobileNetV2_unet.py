...
class MobileNetV2_unet(nn.Module):
    ...
    def forward(self, x):
        ...
        joint = adaptive_avg_pool2d(joint, (16, 2))
        #joint = joint.view(joint.size(0), joint.size(1), -1)
        ...
        if self.mode == "eval":
            x = interpolate(x, scale_factor=2.0,
                            mode='bilinear', align_corners=False)
        #logging.debug((x.shape, 'interpolate'))
        #scale = interpolate(x, scale_factor=2.0,mode='bilinear', align_corners=False)
